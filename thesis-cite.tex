...
\usepackage[natbib=true,style=authortitle,labelyear=true,autocite=footnote]{biblatex}  
...
\input{interferometerSample.pdf_tex}\caption{Interferometer with low coherence light source and micro-structured sample. \label{fig:interferometer}}
%
\end{minipage}\hfill{}%
\begin{minipage}[t]{0.48\columnwidth}%
...
%
%balanced detection\cite{Houser:1994p1780}
...
%
%\cite{Targowski:2004p1713}
%
%
%\subsubsection{Hilbert transform}
%
%In \gls{FDOCT}, the spectral interference signal between the reference light and the light backscattered from the sample at a lateral position $x$ is detected by either a spectrometer (SDOCT) or a point detector (SSOCT) at each single wavelength, akin to the monochromatic laser interferometer. By ignoring the signals that do not contribute to the depth localization in the sample for simplicity, the interferogram detected at each wavelength $\lambda$ can be expressed as
%\begin{equation}
%I\left(k,x\right)=2S\left(k\right)\int\sqrt{R\left(x,z\right)}\cos\left[2kz+\phi\left(k,z\right)\right]dz,\end{equation}
% 
%where $k=2\pi/\lambda$, $z$ is the relative distance between the reference mirror position and the scatters within the sample, $R\left(x,z\right)$ is the normalized intensity backscattered from the scatter at $z$, $S\left(k\right)$ is the spectral density of the light source, and $\phi\left(x,z\right)$ is the phase that relates to the optical heterogeneity of the sample in the focus coherence volume. Here, we assume the reflection in the reference arm to be unity for simplicity. Because the sample can be considered as a phase object, when the OCT probe beam is scanned over in the $x$ direction, $\phi\left(x,z\right)$ is related to the phases of the object at the cross-section plane, i.e., OCT B scan. In practice, at each scanning position, the interference spectrum is discretized by $N$ segments, corresponding to the number of pixels in the camera in SROCT or number of samples captured in one entire sweep in SSOCT. If the probing beam is sampled by $M$ times in the $x$ direction, a data matrix with a size of$\left(N,M\right)$, see figure 1�a�, is built up from which an OCT B scan image is reconstructed by use of an appropriate Fourier transformation (FT) algorithm.
%
%As alluded above, when the probe beam is scanned in the lateral direction, the interference is recorded at each wavelength,	giving	us	N	equations	of	Eq. (1), $I_i\left(k_i,x\right)\left(i=1:N\right)$, with the position $x$ as the variable. The complex function of the OCT interferogram at each $k_i$ can be determined through analytic continuation of the measured interference fringes along the $x$ direction by use of a \fnurl{Hilbert transformation}{} 
%\begin{equation}
%\tilde{I}\left(k_{i},x\right)=I\left(k_{i},x\right)+\frac{i}{\pi}P\int_{-\infty}^{\infty}\frac{I\left(k_{i},x\right)}{\tau-x}d\tau=A\left(k_{i},x\right)\exp\left(i\varphi\left(x\right)\right),\,\left(i=1,\dots,N\right),\end{equation}
%where $P$ denotes	the	Cauchy	principle	value, $A\left(k_{i},x\right)$ is he "envelope" or amplitude and $\varphi\left(x\right)$ is the phase of the analytic signal which is a random fluctuation term that is caused by the heterogeneous properties of scattering sample. We should appreciate that the Hilbert transformation of Eq. does not necessarily represent its quadrature signal because the spectrum of signal bandwidth of $I\left(k_{j},x\right)$ caused by $\varphi\left(x\right)$ is centered at the zero frequency. However, if the Bedrosian theorem holds\cite{Bedrosian:1962p772} which can be explained in our case that if the signal bandwidth caused by $\varphi\left(x\right)$ is sufficiently away from the zero frequency, the Hilbert transformation of Eq. will be strictly equal to its quadrature representation. This can be achieved by introducing a carrier frequency $f_c$ into $I\left(k_{j},x\right)$. With this, the complex function of the spectral OCT interferograms over the B scan matrix is constructed by use of the quadrature representation of Eq. . Next, we perform FT along $k$ at each position $x_j$ to obtain B scan OCT structure image, as usually done in the conventional \gls{FDOCT}.
%\begin{equation}
%I\left(k,x\right)=FT\left[\tilde{I}\left(k_{i},x\right)\right]\,\left(j=1,\dots,N\right),
%\end{equation}
%\cite{Wang:2007p771,Baumann:2007p775}
%
%
%\subsection{Spectral resolution effects}
%
%Acquisition of a spectrum implies that there is a minimum spectral resolution. The consequence of this constraint is that it limits the minimum interference fringe period that can be measured, and therefore, the observable range. Full analysis make use of \fnurl{Shannon sampling theorem}{http://en.wikipedia.org/wiki/Nyquist-Shannon_sampling_theorem}.
%\begin{equation}
%g\left(\nu_{i}\right)=\int_{-\infty}^{\infty}G\left(\nu\right)R\left(\nu-\nu_{i}\right)d\nu\end{equation}
%The measurement range of the \gls{FDOCT} device depends on the spectral resolution of the spectrometer. Practically the useful information about the object structure is deciphered by the numerical Fourier transform FFT of the measured signal. The number of sampling points N is given by the number of illuminated CCD pixels. The dependence between the j-th harmonic and the optical path difference j is the following 
%\begin{equation}
%l_{j}=\frac{1}{2n}\frac{\lambda_{0}^{2}}{\Delta\lambda}j,\, j=1,2,\ldots,N/2\end{equation}
%where n is the refractive index and is the registered width of the spectrum. According to the Nyquist condition the index of the maximum harmonics is equal to N/2. Therefore the maximum path difference registered by a \gls{FDOCT} device is
%\begin{equation}
%l_{\text{max}}=\frac{1}{4\pi}\frac{\lambda_{0}^{2}}{\Delta\lambda}N.\end{equation}
%\section{Processing}
%
%Displacement guessing algorithm
%\begin{equation}
%G^{\left(i\right)}\left(\nu\right)=I_{0}\left(\nu\right)+2S\left(\nu\right)\sum_{n}\cos\left[-2\pi\nu\left(\tau_{n}-\tau_{r}\right)+\Delta_{i}\left(\nu\right)\right]\end{equation}
%\begin{equation}
%=I_{0}\left(\nu\right)+a_{1}\left(\nu\right)\cos\left[\Delta_{i}\left(\nu\right)\right]+a_{2}\left(\nu\right)\sin\left[\Delta_{i}\left(\nu\right)\right],\, i=0,\dots,N.\end{equation}
...
%\begin{equation}
%\begin{cases}
%\left|\mathbf{U}\left(\nu\right)\right|=\sqrt{a_{1}^{2}\left(\nu\right)+a_{2}^{2}\left(\nu\right)}\\
%\arg\left(\mathbf{U}\left(\nu\right)\right)=\arctan\frac{-a_{2}\left(\nu\right)}{a_{1}\left(\nu\right)}\end{cases}\end{equation}
...
% 
%\begin{equation}
%\mathbf{V}\left(\tau\right)=FT^{-1}\left[\mathbf{U}\left(\nu\right)\right]=\Gamma\left(\tau\right)\otimes\sum_{n}\sqrt{\alpha_{n}\beta_{0}}\delta\left(\tau-\left(\tau_{r}-\tau_{n}\right)\right).\end{equation}
...
%\begin{equation}
%\Delta_{0}\left(\nu\right)=0;\,\Delta_{i}\left(\nu\right)=\frac{2\pi}{c}\nu\cdot2\left(\delta z\cdot i+\Delta z_{i}'\right),\, i=1,2,3.\end{equation}
...
A axial resolution of 6 \textgreek{m}m achieved where the theoretical value was 5.6\textgreek{m}m. With 2 mW of optical power incident on the sample, a system sensibility of 98\textpm5 dB was measured, following the procedure of Ref. \cite{Leitgeb:2003p1678}. The data acquisition and image generation were controlled by a LabView supported software. Two spectra full range complex technique was used to eliminate mirror image and DC components. Where the piezo was used to add the quadrature phase shift. 
...
%
%\cite{huber:2006p1731}\fnurl{commercial swept source lasers}{http://www.thorlabs.com/newgrouppage9.cfm?objectgroup_id=1276}
%
%
...
%\subsection{Probe}
%
%\cite{Sun:2010p837}\cite{Yeow:2005p66}\cite{Xie:2003p88}
%
%\section{Signal Processing}
%
...
%A recent study evaluating the enamel-restoration interface using OCT, in which just one patient was reported, has been published and Negritu et al.\cite{Negrutiu:2009p2189} used OCT and confocal microscopy to investigate dental structures and restoration materials using extracted teeth.

%Microleakage, either from small or microscopic openings between the margins of the composite restoration and tooth is considered to be a major cause of restoration failure\cite{Xie:2008p2148}. Microleakage can also result in bacteria penetrating the tooth-restoration space and into dentinal tubules, where secondary decay may occur and bacterial toxins will irritate the pulp. The oral environment, including occlusal forces and temperature variation, as well as the differences between the physical properties of teeth and restorative materials, polymerization shrinkage, the coefficient of thermal expansion, and modulus of elasticity can all contribute to microleakage. According to previous literature, if poor bond strength exists between the tooth and restorative material, a failure of adhesion may be caused by polymerization shrinkage, and microscopic gaps at the tooth-restoration interface can subsequently form \cite{DeMelo:2005p2100,Attar:2008p2147,Xie:2008p2148}.

%OCT has already been applied in vitro and in vivo to a variety of medical specialties, including Opthalmology\cite{Huang:1991p1955} , Dermatology \cite{Pierce:2004p1839}, Gastroenterology \cite{Tearney:1996p2052}, Cardiology\cite{Boppart:1997p1840} and Cellular Engineering \cite{Yang:2006p2053}.
%
%The optical coherence tomography OCT consists of a new imaging technique for diagnosis that produces bi or three-dimensional pictures, with a few microns of spatial resolution, and has been widely used to study biotissues, as reviewed\cite{Bouma:2002p2007}. This technique was initially used in ophthalmology\cite{Huang:1991p1955}, where it is well developed and clinically used, but it has been continuously benefiting other areas, such as cardiology\cite{Boppart:1997p1840}, gastroenterology\cite{Tearney:1996p2052}, dermatology\cite{Pierce:2004p1839}, and cellular engineering\cite{Yang:2006p2053}. In dentistry, the first applications were reported in 1998\cite{Colston:1998p1677}, having been used for diverse purposes such as characterization of periodontal structures\cite{Otis:2000p2284,Colston:1995p1719}, recurrent carie detection of and marginal adaptation of restorations\cite{DeMelo:2005p2100}, and precocious detention of oral cancer\cite{Jung:2005p2056}. 
...
%In recent years, several methods using light have been proposed to address the need for better detection and diagnostic tools in dentistry. Digital imaging fiber-optic transillumination (DIFOTI), multi-photon imaging, quantitative light fluorescence (QLF), infrared fluorescence (DIAGNOdent), infrared thermography, Raman spectroscopy, optical coherence tomography (OCT) and terahertz imaging are a few techniques that are currently under evaluation for improved diagnosis\cite{Hall:2004p1838,Fried:2005p1836,Alex:2005p1695,ChooSmith:2008p1834}.

%The first study applying OCT to be carried out in Dentistry, both in vitro and in vivo, was in 1998, using a prototype handpiece for the characterization of both hard and soft dental tissues \cite{Colston:1995p1719,Feldchtein:1996p111}. In 2003, a commercial prototype OCT system was adapted to a commercial surgical microscope to perform in vitro and in vivo images of carious lesions and restorations \cite{Brandenburg:2003p2055}. OCT has also been employed for the early detection of oral cancer \cite{Jung:2005p2056}. In vitro studies evaluating enamel interface restoration \cite{DeMelo:2005p2100}, early caries diagnostics \cite{Freitas:2009p2142}, and analysis of the performance of dental materials\cite{Braz:2009p2143} have also been reported. In 2006, the first OCT image of dental pulp was performed using rat's teeth \cite{Kauffman:2006p2144}. Recently, remaining dentin and the pulp chamber from human teeth were also imaged by OCT in vitro as reported in \cite{Fonseca:2009p1}. Endodontic applications of OCT have been explored in order to evaluate root canals \cite{Shemesh:2007p1843} and Sinescu and co-workers used OCT to investigate the marginal adaptation of fixed dental prosthesis before installing them in patients \cite{Sinescu:2009p2146}. However, little work has been reported on OCT being used in a clinical setting as part of a clinical evaluation, specifically looking at restoration integrity.
...
%Previous studies have shown some defects we have identified in this paper: the dentin enamel junction was evidenced by several authors\cite{Colston:1995p1719,Feldchtein:1996p111,Brandenburg:2003p2055}; Melo et al (2005)\cite{DeMelo:2005p2100} presented induced marginal microleakage; Sadr et al (2009)\cite{24} had shown the evolution of a marginal microleakage to a secondary caries with absence of restorative material under the restoration; the absence of restorative material in the interface between the restoration and the dental tissue (similar to that we have shown in figure 5), presence of gaps, as well as crack points of the restorative material were presented by Negritu et al (2009)\cite{Negrutiu:2009p2189}; air bubble and gaps were also presented in the study performed by Feldchtein et al (1998)\cite{Feldchtein:1996p111} and, finally, crack propagation into the resin composite was described by Braz et al (2009)\cite{Braz:2009p2143}.
...
%Attempts to fabricate on commercial foundries has been made\cite{Orcutt:2008p1769}
...
%\cite{Miyagi:1978p1770,Baets:1983p1772,Hutcheson:1980p1771}
...
%gallium arsenide. The relative errors are estimated as $\pm15\%$.\cite{Dinu:2003p1592}}
%
%\end{table}
%
%
...
%\caption{(a) Electro-absorption and (b) electro-refraction versus wavelength in c-Si. Electro-absorption horizontal axis is linear in photon energy.\cite{Soref:1987p1492}}
%\end{figure}
...
%
%Multi-aperture planar waveguide spectrometer formed by arrayed Mach-Zehnder interferometers\cite{Florjanczyk:2007p1602}
%
...
We fabricate the device using a CMOS compatible process. We start with a silicon-on-insulator (SOI), manufactured by \fnurl{Soitec}{http://www.soitec.com} wafer with a 250 nm top silicon layer and a 3 \textgreek{m}m buried oxide layer. A 60 nm layer of \ce{SiO2} is deposited using high-temperature low-pressure chemical vapor deposition (HTO) to be used as a hard mask. The grating, ring and waveguides are defined by e-beam lithography on a PMMA resist mask. The pattern is transferred to the oxide layer using a \ce{CHF3}/\ce{O2} reactive ion etch (RIE). The silicon layer is etched using chlorine RIE.
...
The fabrication of the dielectric part was carried the same way as the DG spectrometer, described in \ref{section:fabrication 1}. An alternation was done between before cladding the device with PECVD silicon oxide, which was to first cover de device conformally with 160 nm of \ce{SiO2} using a chemical vapor deposition at 1200\textdegree C, this was done because \ce{SiO2} deposited using PECVD was not completely filling the gap in the coupling to the ring waveguides. Here we only describe the additional steps to produce the ring heater.
...
%	\end{minipage}\hfill{}%
%	\begin{minipage}[t]{0.49\columnwidth}%
%		\includegraphics{tempshift-fit}\caption{Resonance wavelengths as temperature increases and fitted linear function.\label{}}
	\end{minipage}
\end{figure}
